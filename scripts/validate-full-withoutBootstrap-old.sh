# Perform the tests of an issue with standard slice/configuraiton loading using Monticello and then bootstrap from the source codes
# Arguments:
#   - issue id (or "next" for the next issue ready for validation)
#   - Pharo version number
#   - Pharo version number used for exporting
# Examples:
#   bash validate.sh next 60 
#   bash validate 19488 60

VM_PATH="$PWD/pharo"
MONKEY_IMAGE_NAME=Pharo
TESTS_IMAGE_NAME=tests
ISSUE=$1
INPUT_IMAGE_NAME=core
